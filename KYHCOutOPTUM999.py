...
f = open("C:/Users/joseph.petitt/Documents/SFTP_Files/Working/KYHCOutOPTUM999DE.txt","w") #Enter the txt file you want the parsed 837's to be written to
if __name__ == '__main__':
    # Sample usage

    
    path = 'C:/Users/joseph.petitt/Documents/SFTP_Files/CenterCare_KYHC_PROD/Inbound/Archive/OPTUM*.999' #enter the directory to run the script on
    ...
    for File in files:
        ...
        for segment in message:
            ...
            if segment.find("ISA",0,3) != -1: #Find the ISA segment and get the Interchange Control Number, write the ISA segment to the file with delimiters
                ...
                continue
            if segment.find("GS",0,2) != -1:  #find the GS segment and get the Group Control Number, write the GS segment to the file with delimiters
                GS = segment.split("*")
            ...
            if segment.find("ST",0,2) != -1:   #find the ST segement and get the Transaction Control Number as set the Transaction Line Number
                ST = segment.split("*")
            ...
            if segment.find("AK1",0,3) != -1: 
                AK1 = segment.split("*")
                AK1_Index_1 = AK1[1]
                AK1_Index_2 = AK1[2]
                AK1_Index_3 = AK1[3]
                f.write(fileName + "{" + str(lineNumber) + "{" + Interchange_Ctrl_Nbr + "{" + Group_Control_Nbr + "{" + Transaction_Ctrl_Nbr + "{" + str(Transaction_Line_Nbr) + "{" + AK1_Index_1 + "{" + AK1_Index_2 + "{" + AK1_Index_3 + "{{{{" + ST_Segment + "~" + "{" + ST[0] + "{" + ST[1] + "{" + ST[2] + "{" + ST[3] + "{{{{{{{{{{{{{" + "\n")
                lineNumber += 1
                Transaction_Line_Nbr += 1
                f.write(fileName + "{" + str(lineNumber) + "{" + Interchange_Ctrl_Nbr + "{" + Group_Control_Nbr + "{" + Transaction_Ctrl_Nbr + "{" + str(Transaction_Line_Nbr) + "{" + AK1_Index_1 + "{" + AK1_Index_2 + "{" + AK1_Index_3 + "{{{{" + segment + "~" + "{" + AK1[0] + "{" + AK1[1] + "{" + AK1[2] + "{" + AK1[3] + "{{{{{{{{{{{{{" + "\n")
                lineNumber += 1
                Transaction_Line_Nbr += 1
                continue

            if segment.find("AK2",0,3) != -1:
                AK2 = segment.split("*")
                AK2_Index_1 = AK2[1]
                AK2_Index_2 = AK2[2]
                AK2_Index_3 = AK2[3]
            ...
            if segment.find("GE",0,2) != -1: 
                GE = segment.split("*")
                f.write(fileName + "{" + str(lineNumber) + "{" + Interchange_Ctrl_Nbr + "{" + Group_Control_Nbr + "{{{{{{" + AK2_Index_1 + "{" + AK2_Index_2 + "{" + AK2_Index_3 + "{" + segment + "~" + "{" + GE[0] + "{" + GE[1] + "{" + GE[2] + "{{{{{{{{{{{{{{" + "\n")
                lineNumber += 1
                continue
            if segment.find("IEA",0,3) != -1:
                IEA = segment.split("*")
                f.write(fileName + "{" + str(lineNumber) + "{" + Interchange_Ctrl_Nbr + "{{{{{{{" + AK2_Index_1 + "{" + AK2_Index_2 + "{" + AK2_Index_3 + "{" + segment + "~" + "{" + IEA[0] + "{" + IEA[1] + "{" + IEA[2] + "{{{{{{{{{{{{{{" + "\n")
                continue

            DE = segment.split("*") 
            while len(DE) != 16: #if a segement has less than 17 data elements add blanks, this allows printing of the correct amount of delimeters
                DE.append("")
            
            f.write(fileName + "{" + str(lineNumber) + "{" + Interchange_Ctrl_Nbr + "{" + Group_Control_Nbr + "{" + Transaction_Ctrl_Nbr + "{" + str(Transaction_Line_Nbr) + "{" + AK1_Index_1 + "{" + AK1_Index_2 + "{" + AK1_Index_3 + "{" + AK2_Index_1 + "{" + AK2_Index_2 + "{" + AK2_Index_3 + "{" + segment + "~" + "{" + DE[0] + "{" + DE[1] + "{" + DE[2] + "{" + DE[3] + "{" + DE[4] + "{" + DE[5] + "{" + DE[6] + "{" + DE[7] + "{" + DE[8] + "{" + DE[9] + "{" + DE[10] + "{" + DE[11] + "{" + DE[12] + "{" + DE[13] +"{" + DE[14] + "{" + DE[15] + "{" + "\n")
            lineNumber += 1
            Transaction_Line_Nbr += 1
