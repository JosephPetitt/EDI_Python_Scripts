...
f = open("C:/Users/joseph.petitt/Documents/SFTP_Files/KYHCOutOPTUM277CADE.txt","w") #Enter the txt file you want the parsed 837's to be written to
if __name__ == '__main__':
    # Sample usage

    
    path = 'C:/Users/joseph.petitt/Documents/SFTP_Files/Working/OPTUM*.277CA' #enter the directory to run the script on
    ...
    for File in files:
        ...
        for segment in message:
            ...
            DE = segment.split("*") 
            while len(DE) != 16: #if a segement has less than 31 data elements add blanks, this allows printing of the correct amount of delimeters
                DE.append("")
            
            f.write(fileName + "{" + str(lineNumber) + "{" + Interchange_Ctrl_Nbr + "{" + Group_Control_Nbr + "{" + Transaction_Ctrl_Nbr + "{" + str(Transaction_Line_Nbr) + "{" + segment + "~" + "{" + DE[0] + "{" + DE[1] + "{" + DE[2] + "{" + DE[3] + "{" + DE[4] + "{" + DE[5] + "{" + DE[6] + "{" + DE[7] + "{" + DE[8] + "{" + DE[9] + "{" + DE[10] + "{" + DE[11] + "{" + DE[12] + "{" + DE[13] +"{" + DE[14] + "{" + DE[15] + "{" + "\n")
            lineNumber += 1
            Transaction_Line_Nbr += 1
